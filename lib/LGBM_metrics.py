...
ERROR_METRICS = ["RMSE", "MAE", "MAPE", "LIKELIHOOD"]
...
class ErrorsInfo:
    ...
    def calculate_errors(
        self, test_df: pd.DataFrame, target_col: str
    ):
        ...
        for error_metric in errors_info_keys_generator():
            ...
            self.errors_info[(error_metric)] = calculate_error_metric(
                test_df, error_metric, target_col
            )
    ...
    def calculate_avg_test_errors(
        ...
        for error_metric in errors_info_keys_generator():
            ...
            self.errors_info[(error_metric)] = calculate_error_metric(
                concated_test_df, error_metric, target_col
            )
    ...
    def calculate_avg_train_errors(
        ...
        for error_metric in errors_info_keys_generator():
            ...
            self.errors_info[(error_metric)] = statistics.mean(
                [
                    train_errors_info.errors_info[(error_metric)]
    ...
    def calculate_avg_errors(
        ...
        for test_df in test_dfs:
            ...
            filtered_test_dfs += test_df[f"prediction"]
        ...
        for error_metric in errors_info_keys_generator():
            ...
            self.errors_info[(error_metric)] = calculate_error_metric(
                filtered_test_df, error_metric, target_col
            )
    ...
    def print_error(
        ...
        if (error_metric) not in self.errors_info:
            return

        value = self.errors_info[(error_metric)]
        ...
        if base_errors_infos is not None:
            ...
            for base_errors_info in base_errors_infos:
                if (error_metric) in base_errors_info.errors_info:
                ...
                gains += str(
                    f"{beg_color}" + f'({"%.2f"%gain})' + f"{end_color}" + f"    "
                )
...
class ClassErrorsInfo:
    ...
    def class_calculate_errors(
        ...
        self.target_avg = np.mean(test_df[target_col])
        self.errors_info = {}
        for error_metric in class_errors_info_keys_generator():
    ...
    def calculate_avg_errors(
        ...
        filtered_test_df = pd.concat(filtered_test_dfs)
        prediction_classification = np.concatenate(prediction_classification, axis=0)
