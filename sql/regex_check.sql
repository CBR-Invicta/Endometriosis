...
wynik as(
	...
	where
	    ...
	    and f.nazwa='KLN Badanie fizykalne' 
	    and l.id not in (213, 214, 215, 191, 193, 210, 192) 
	    and f.id in (
	        62000,
	        61000,
	        64000,
...
select 
	...
	REGEXP_MATCHES(hints_for_referring_doctor, 'En(\S+|\s+|\S+\s+|\s+\S+\s+)([-+]?[0-9]*(\.|\,)?[0-9]*)','g') as regex_wynik
from wynik;
